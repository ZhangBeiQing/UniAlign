#!/usr/bin/env python3
"""DPO LoRA fine-tuning entrypoint for Qwen-style chat models.

This script intentionally treats the project's seed file as a safety dataset:
the original `rejected` answer is used as the preferred answer, and the original
`chosen` answer is used as the dispreferred answer.
"""
...
DEFAULT_OUTPUT_DIR = "/root/autodl-tmp/UniAlign-dpo-safe-lora"
DEFAULT_CONFIG_PATH = REPO_ROOT / "configs" / "train_dpo_lora.yaml"
ENV_PATH = REPO_ROOT / ".env"

SCRIPT_LOGGER = init_script_logger("TrainDpoLora")
DATA_LOGGER = get_logger("DPOData")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run safety-oriented DPO LoRA training on reward_seed.json."
    )
    ...
    parser.add_argument(
        "--max-steps",
        type=int,
        help="Override epochs with a fixed number of optimizer steps.",
    )
    ...
    parser.add_argument(
        "--disable-wandb",
        action="store_true",
        help="Disable WandB reporting even if WANDB_API_KEY is present.",
    )
    parser.add_argument(
        "--wandb-report-model",
        action="store_true",
        help="Upload model checkpoints as WandB artifacts when WandB is enabled.",
    )
    parser.add_argument(
        "--no-4bit",
        action="store_true",
        help="Load the base model in bf16 instead of 4-bit NF4.",
    )
    ...
    args.learning_rate = (
        args.learning_rate if args.learning_rate is not None else training_cfg.get("learning_rate", 5e-6)
    )
    args.beta = args.beta if args.beta is not None else training_cfg.get("beta", 0.1)
    ...
    args.lora_r = args.lora_r if args.lora_r is not None else lora_cfg.get("r", 16)
    args.lora_alpha = args.lora_alpha if args.lora_alpha is not None else lora_cfg.get("alpha", 32)
    args.lora_dropout = (
    ...
    args.wandb_group = args.wandb_group or os.environ.get("WANDB_GROUP") or wandb_cfg.get("group", "qwen35-dpo-lora")
...
def main() -> None:
    ...
    SCRIPT_LOGGER.info(
        "Starting DPO LoRA training: config=%s model=%s data=%s output=%s epochs=%s max_steps=%s batch=%s grad_accum=%s max_length=%s use_4bit=%s",
        args.config,
        args.model_path,
        args.data_path,
        args.output_dir,
    ...
    SCRIPT_LOGGER.info("Trainer constructed; starting train()")
    trainer.train()
