#!/usr/bin/env python3
"""Chat with a local Hugging Face causal LM plus an optional LoRA adapter."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Chat with a local Hugging Face causal LM and optional LoRA adapter.")
    parser.add_argument("--model-path", type=str, default=DEFAULT_MODEL_PATH)
    parser.add_argument("--lora-path", type=str, default=DEFAULT_LORA_PATH)
    parser.add_argument("--base-only", action="store_true", help="Skip loading the LoRA adapter.")
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.lora_path if not args.base_only else args.model_path, trust_remote_code=True)
